...
def process_query_websocket(event, connection_id, domain_name, stage, context):
    ...
    try:
        ...
        processing_message = get_processing_message(language)
...
def process_query_async(event, context):
    """
    Process the query asynchronously with enhanced timeout and error handling
    """
    ...
    try:
        logger.info(f"ASYNC_START: Processing started for connection {connection_id}, user {user_id}")
        logger.info(f"ASYNC_REMAINING_TIME: {context.get_remaining_time_in_millis() / 1000:.1f}s")
        ...
        # Check if connection is still active before starting processing
        if not check_connection_active(connection_id, domain_name, stage):
            logger.warning(f"Connection {connection_id} is no longer active")
            return {'statusCode': 200}
        ...
        if services and len(services) > 0:
            ...
            # Check connection again before long operation
            if not check_connection_active(connection_id, domain_name, stage):
                logger.warning(f"Connection {connection_id} lost before Perplexity call")
                return {'statusCode': 200}

            # Send a status update before calling Perplexity
            waiting_message = get_waiting_message(language)
            if not send_to_connection(connection_id, domain_name, stage, {
                'user_id': user_id,
                'status': 'searching',
            ...
            # Call Perplexity with enhanced timeout handling
            try:
                logger.info(f"PERPLEXITY_START: Calling Perplexity Lambda for user {user_id}")
                perplexity_start = time.time()

                perplexity_response = call_perplexity_lambda_with_timeout(user_query, user_id, actual_zipcode, language)

                perplexity_duration = time.time() - perplexity_start
                logger.info(f"PERPLEXITY_END: Perplexity call completed in {perplexity_duration:.2f}s")

                # Handle different response scenarios
                if isinstance(perplexity_response, dict):
                    status_code = perplexity_response.get('statusCode', 200)

                    if status_code == 408:  # Request Timeout
                        logger.info("PERPLEXITY_TIMEOUT: Received timeout response from Perplexity")
                        send_timeout_response(connection_id, domain_name, stage, user_id, actual_zipcode, language)

                    elif status_code >= 500:  # Server Error
                        logger.error(f"PERPLEXITY_ERROR: Server error {status_code}")
                        send_service_error_response(connection_id, domain_name, stage, user_id, actual_zipcode, language)

                    elif status_code == 200:  # Success
                        # Parse and send successful Perplexity response
                        body = perplexity_response.get('body', '{}')
                        if isinstance(body, str):
                            try:
                                ...
                                logger.error("Failed to parse Perplexity response body")
                                send_general_error_response(connection_id, domain_name, stage, user_id, actual_zipcode, language, "parse_error")
                        else:
                            perplexity_response['status'] = 'complete'
                            if not send_to_connection(connection_id, domain_name, stage, perplexity_response):
                                logger.warning(f"Failed to send Perplexity results - connection {connection_id} disconnected")
                    else:
                        logger.warning(f"Unexpected status code from Perplexity: {status_code}")
                        send_general_error_response(connection_id, domain_name, stage, user_id, actual_zipcode, language, "unexpected_status")

                else:
                    logger.warning(f"Unexpected Perplexity response format: {type(perplexity_response)}")
                    send_general_error_response(connection_id, domain_name, stage, user_id, actual_zipcode, language, "unexpected_response")

            except Exception as perplexity_error:
                logger.error(f"Error calling Perplexity: {str(perplexity_error)}")
                logger.error(traceback.format_exc())
                send_general_error_response(connection_id, domain_name, stage, user_id, actual_zipcode, language, "perplexity_error")

            return {'statusCode': 200}

    except Exception as e:
        duration = time.time() - start_time
        logger.error(f"Error in async processing after {duration:.2f}s: {str(e)}")
        logger.error(traceback.format_exc())

        try:
            send_general_error_response(connection_id, domain_name, stage,
                              body.get('user_id', 'unknown') if 'body' in locals() else 'unknown',
                              body.get('zipcode') if 'body' in locals() else None,
                              body.get('language', 'english').lower() if 'body' in locals() else 'english',
                              "processing_error")
        except:
...
def check_connection_active(connection_id, domain_name, stage):
    """
    ...
    try:
        ...
        api_gateway_management_client.get_connection(ConnectionId=connection_id)
        return True
    except Exception as e:
        if hasattr(e, 'response') and e.response.get('ResponseMetadata', {}).get('HTTPStatusCode') == 410:
            logger.info(f"Connection {connection_id} is gone (410)")
            return False
        logger.warning(f"Could not verify connection status for {connection_id}: {str(e)}")
        return True  # Assume active if we can't determine

def call_perplexity_lambda_with_timeout(user_query, user_id, zipcode=None, language='english'):
    """
    Call the Perplexity Lambda function with enhanced error handling
    """
    try:
        logger.info(f"Calling Perplexity Lambda for query: {user_query[:50]}...")
        ...
        logger.info(f"Perplexity Lambda response status: {response_payload.get('statusCode', 'unknown')}")
        ...
        return {
            ...
            "body": json.dumps({
                "status": "error",
                "message": f"Error calling Perplexity service: {str(e)}",
                "error_type": "lambda_invocation_error"
            })
        }

def get_processing_message(language):
    """Get processing message in appropriate language"""
    messages = {
        'spanish': "Su solicitud está siendo procesada. Esto puede tomar un momento, por favor espere...",
        'polish': "Twoje zapytanie jest przetwarzane. To może chwilę potrwać, proszę czekać...",
        'english': "Your request is being processed. This may take a moment, please wait..."
    }
    return messages.get(language.lower(), messages['english'])

def get_waiting_message(language):
    """Get waiting message in appropriate language"""
    messages = {
        'spanish': "Buscando recursos especializados. Esto puede tomar un momento...",
        'polish': "Szukam specjalistycznych zasobów. To może chwilę potrwać...",
        'english': "Searching for specialized resources. This may take a moment..."
    }
    return messages.get(language.lower(), messages['english'])

def get_timeout_message(language):
    """Get timeout message in appropriate language"""
    messages = {
        'spanish': "La búsqueda está tomando más tiempo de lo esperado. Por favor intenta de nuevo en unos momentos.",
        'polish': "Wyszukiwanie trwa dłużej niż oczekiwano. Spróbuj ponownie za chwilę.",
        'english': "The search is taking longer than expected. Please try again in a few moments."
    }
    return messages.get(language.lower(), messages['english'])

def get_service_error_message(language):
    """Get service error message in appropriate language"""
    messages = {
        'spanish': "El servicio de búsqueda no está disponible temporalmente. Por favor intenta de nuevo más tarde.",
        'polish': "Usługa wyszukiwania jest tymczasowo niedostępna. Spróbuj ponownie później.",
        'english': "The search service is temporarily unavailable. Please try again later."
    }
    return messages.get(language.lower(), messages['english'])

def get_general_error_message(language):
    """Get general error message in appropriate language"""
    messages = {
        'spanish': "Ocurrió un error al procesar tu solicitud. Por favor intenta de nuevo.",
        'polish': "Wystąpił błąd podczas przetwarzania Twojego żądania. Spróbuj ponownie.",
        'english': "An error occurred while processing your request. Please try again."
    }
    return messages.get(language.lower(), messages['english'])

def send_timeout_response(connection_id, domain_name, stage, user_id, zipcode, language):
    """Send timeout-specific response"""
    message = get_timeout_message(language)
    send_to_connection(connection_id, domain_name, stage, {
        'user_id': user_id,
        'status': 'timeout',
        'message': message,
...
def send_service_error_response(connection_id, domain_name, stage, user_id, zipcode, language):
    """Send service error response"""
    message = get_service_error_message(language)
    send_to_connection(connection_id, domain_name, stage, {
        'user_id': user_id,
        'status': 'error',
        'message': message,
...
def send_general_error_response(connection_id, domain_name, stage, user_id, zipcode, language, error_type):
    """Send general error response"""
    message = get_general_error_message(language)
    send_to_connection(connection_id, domain_name, stage, {
        'user_id': user_id,
        'status': 'error',
        'message': message,
...
def send_to_connection(connection_id, domain_name, stage, data):
    ...
    try:
        ...
        message_data = json.dumps(data, cls=DecimalEncoder).encode('utf-8')

        api_gateway_management_client.post_to_connection(
            ConnectionId=connection_id,
            Data=message_data
        )
        logger.info(f"Message sent to connection {connection_id}: {data.get('status', 'unknown')}")
        return True
...
def call_perplexity_lambda(user_query, user_id, zipcode=None, language='english'):
    ...
    (Maintained for backward compatibility - use call_perplexity_lambda_with_timeout for new code)
    ...
    """
    return call_perplexity_lambda_with_timeout(user_query, user_id, zipcode, language)
