...
HTTP_TIMEOUT = int(os.environ.get('HTTP_TIMEOUT', '45'))  # 45 seconds default
MAX_PROCESSING_TIME = int(os.environ.get('MAX_PROCESSING_TIME', '80'))  # 80 seconds default
...
def timeout_context(seconds):
    """Context manager for setting socket timeout"""
...
def get_timeout_message(language: str) -> str:
    """Get appropriate timeout message based on language"""
    messages = {
...
def get_error_message(language: str) -> str:
    ...
    messages = {
        'spanish': "Ocurrió un error al buscar servicios. Por favor intenta de nuevo.",
        'polish': "Wystąpił błąd podczas wyszukiwania usług. Spróbuj ponownie.",
        'english': "An error occurred while searching for services. Please try again."
    }
    return messages.get(language.lower(), messages['english'])

def translate_response_content(response_data: Dict[str, Any], target_language_code: str) -> Dict[str, Any]:
    """
    Translate the content of a response to the target language with optimization
    ...
    try:
        ...
        # If target language is English, just update the language field and return
        if target_language_code == 'en':
            translated_response['language'] = 'english'
...
def query_perplexity(user_query: str, zipcode: Optional[str] = None, language: str = 'english') -> Dict[str, Any]:
    """
    Query the Perplexity API with proper timeout handling - CRITICAL FIX
    ...
    try:
        print(f"PERPLEXITY_START: Starting request with {HTTP_TIMEOUT}s timeout")
        print(f"Query: {user_query[:100]}{'...' if len(user_query) > 100 else ''}")
        print(f"Language: {language}, Zipcode: {zipcode}")
...
When you don't know the answer to something, acknowledge it rather than making up information."""
        ...
        **Second Organization**
        ...
        # Using urllib with proper timeout handling
        ...
        print(f"Making HTTP request to Perplexity API...")

        # CRITICAL FIX: Add timeout to prevent hanging
        with timeout_context(HTTP_TIMEOUT):
            req = urllib.request.Request(PERPLEXITY_API_URL, data=data, headers=headers, method="POST")

            try:
                # This was the source of hanging - now has proper timeout
                with urllib.request.urlopen(req, timeout=HTTP_TIMEOUT) as response:
                    response_data = response.read().decode('utf-8')

                    duration = time.time() - start_time
                    print(f"PERPLEXITY_SUCCESS: Request completed in {duration:.2f}s")
                ...
                duration = time.time() - start_time
                print(f"PERPLEXITY_TIMEOUT: Request timed out after {duration:.2f}s")
                ...
                print(f"PERPLEXITY_HTTP_ERROR: {e.code} after {duration:.2f}s - {error_body}")
                ...
                duration = time.time() - start_time
                print(f"PERPLEXITY_URL_ERROR: {e.reason} after {duration:.2f}s")
                ...
                duration = time.time() - start_time
                print(f"PERPLEXITY_REQUEST_ERROR: {str(e)} after {duration:.2f}s")
        ...
        print(f"PERPLEXITY_GENERAL_ERROR: {str(e)} after {duration:.2f}s")
        return {
            "error": "general_error",
            "message": get_error_message(language),
            "duration": duration
...
def create_success_response(user_id, zipcode, language, response_data):
...
def lambda_handler(event, context):
    """
    Enhanced lambda handler with proper timeout and error handling
    """
    start_time = time.time()

    try:
        print(f"LAMBDA_START: Processing Perplexity request")
        print(f"LAMBDA_REMAINING_TIME: {context.get_remaining_time_in_millis() / 1000:.1f}s")
        ...
        if not user_query_original:
            print("ERROR: Missing user_query parameter")
            return create_error_response(400, "user_query parameter is required")
        ...
        if language_code != 'en':
            print(f"Translating query from {language_code} to English")
        ...
        print("Checking for cached response in DynamoDB")
        cached_response = query_dynamodb_for_cached_response(user_query, zipcode, language_code)

        if cached_response:
            print("CACHE_HIT: Found cached response in DynamoDB")
            ...
            duration = time.time() - start_time
            print(f"LAMBDA_SUCCESS_CACHED: Completed in {duration:.2f}s")

            return create_success_response(user_id, zipcode, language_name, translated_response)

        # If not in cache, query Perplexity with the English query but request response in original language
        print("CACHE_MISS: No cached response found, querying Perplexity")
        ...
        if remaining_time_ms < 15000:  # Less than 15 seconds left
            print(f"LAMBDA_TIMEOUT_WARNING: Only {remaining_time_ms/1000:.1f}s remaining, not enough time for Perplexity call")
            return create_error_response(408, get_timeout_message(language_name), "lambda_timeout")

        perplexity_response = query_perplexity(user_query, zipcode, language_name)

        # Handle error responses from Perplexity
        if isinstance(perplexity_response, dict) and "error" in perplexity_response:
            error_type = perplexity_response.get("error")
            message = perplexity_response.get("message", "Service temporarily unavailable")
            duration = perplexity_response.get("duration", 0)

            print(f"PERPLEXITY_ERROR: {error_type} after {duration:.2f}s")

            if error_type == "timeout":
                return create_error_response(408, message, "perplexity_timeout")
            elif error_type.startswith("http_"):
                return create_error_response(502, message, error_type)
            else:
                return create_error_response(500, message, error_type)

        # Extract and format the response - pass the user_id for referral tracking
        print("Processing Perplexity response")
        formatted_response = extract_meaningful_response(perplexity_response, language_name, zipcode, user_query, user_id)

        # Check remaining time before database operations
        remaining_time_ms = context.get_remaining_time_in_millis()
        if remaining_time_ms > 10000:  # At least 10 seconds left
            print("Storing response in cache and updating user history")
            ...
            print(f"LAMBDA_TIME_WARNING: Only {remaining_time_ms/1000:.1f}s remaining, skipping database operations")

        duration = time.time() - start_time
        print(f"LAMBDA_SUCCESS: Completed in {duration:.2f}s")
        print(f"LAMBDA_REMAINING_TIME: {context.get_remaining_time_in_millis() / 1000:.1f}s")

        return create_success_response(user_id, zipcode, language_name, formatted_response)

    except Exception as e:
        duration = time.time() - start_time
        remaining_time_ms = context.get_remaining_time_in_millis() if context else 0

        print(f"LAMBDA_ERROR: {str(e)} after {duration:.2f}s")
        print(f"LAMBDA_REMAINING_TIME: {remaining_time_ms / 1000:.1f}s")

        # Determine appropriate error message based on context
        if 'language_name' in locals():
            error_message = get_error_message(language_name)
        else:
            error_message = "An unexpected error occurred"

        return create_error_response(500, error_message, str(e))