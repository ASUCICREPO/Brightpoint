...
def query_dynamodb_for_services(service_categories: Optional[List[str]], zipcode: Optional[str]) -> List[Dict[str, Any]]:
    """
    Query DynamoDB for services matching the given categories and zipcode

    Args:
        service_categories: List of service categories to search for
        zipcode: Optional zipcode to filter by

    Returns:
        List of matching service items
    """
    try:
        # Initialize for storing all matching items
        all_results = []

        # If zipcode is provided, use it to filter
        if zipcode:
            scan_params = {
                'FilterExpression': '',
            ...
            # Add filter for zipcode - check both string and numeric representations
            zipcode_expression = "(#zipcode_str = :zipcode_str OR #zipcode_num = :zipcode_num)"
            filter_expressions.append(zipcode_expression)

            scan_params['ExpressionAttributeNames']['#zipcode_str'] = 'Zipcode'
            scan_params['ExpressionAttributeNames']['#zipcode_num'] = 'Service Area Zip Code'
            scan_params['ExpressionAttributeValues'][':zipcode_str'] = {'S': zipcode}
            scan_params['ExpressionAttributeValues'][':zipcode_num'] = {'N': zipcode}

            # Add service category filters if provided
            if service_categories and len(service_categories) > 0:
                ...
                for i, category in enumerate(service_categories):
                    ...
                    category_conditions.append(f"contains(#service_category_type, {category_key})")
                    scan_params['ExpressionAttributeValues'][category_key] = {"S": category}

                # Add service category attribute name
                scan_params['ExpressionAttributeNames']['#service_category_type'] = 'Service Category Type'

                # Add the category filter expression
                if category_conditions:
                    category_expression = f"({' OR '.join(category_conditions)})"
                    filter_expressions.append(category_expression)
        ...
        # If no filters were provided, return empty list
        else:
            return []

        # Unmarshall items to regular Python dictionaries
    ...
    try:
        ...
        if not user_exists:
            ...
            for service in services:
                # Handle the Unicode BOM in the Agency field name
                agency_name = service.get('\ufeffAgency', service.get('Agency', 'Unnamed Service'))
                address = service.get('Address', '')
                zipcode = service.get('Zipcode', '')
                service_category = service.get('Service Category', '')
                ...
                if not referral_id:
                    print(f"No referral_id found for service {agency_name}. Skipping.")
                    continue
            ...
            for service in services:
                try:
                    ...
                    print(f"Error adding referral for service {agency_name}: {str(e)}")
    ...
    try:
        ...
        for i, service in enumerate(services):
            ...
            for key, value in service.items():
                if key not in ['\ufeffAgency', 'Agency', 'Referral Process', 'Hours',
